...
void startGame(int client_socket) {
    ...
    for (attempts = 1; attempts <= 10; attempts++) {
        ...
        // 오답 문구 제작
        char wrong_message[100];
        snprintf(wrong_message, sizeof(wrong_message), "10번 시도 동안 정답을 맞추지 못했습니다. 정답은 %d %d %d 입니다.\n", randValues[0], randValues[1], randValues[2]);

        //오답 문구 제작
        if (strike != 3) {
            send(client_socket, wrong_message, sizeof(wrong_message), 0);
            players[numPlayers].attempts = attempts;
            numPlayers++;
        }
